...
2. **Locate journal.md.** Check `docs/journal.md`, then `journal.md` in project root. If found, note the path – all other commands (`/user:save`, `/user:handoff`) write to this same location. If none exists, state that.
...
If `$ARGUMENTS` is provided, focus the orientation on that aspect (e.g., `/user:orient requirements` to check only requirements status): $ARGUMENTS
